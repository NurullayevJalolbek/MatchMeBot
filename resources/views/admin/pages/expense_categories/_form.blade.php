...
@section('title', $label ?? 'Xarajat Kategoriyalari')

@section('content')
@php
    $model = $model ?? new \App\Models\ExpenseCategory();
    $isCreate = !isset($method) || strtoupper($method) === 'POST';
    $label = $label ?? ($isCreate ? 'Yangi Xarajat Kategoriyasi' : 'Xarajat Kategoriyasini Tahrirlash');
            ...
            <div class="card stretch stretch-full">
                <div class="card-header">
                    <h5 class="card-title mb-0">
                        ...
                        @else
                            Xarajat Kategoriyasi Parametrlari
                        @endif
                    </h5>
                </div>
                <div class="card-body">
                    <form action="{{ $route }}" method="POST">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="name" value="{{ old('name', $model->name) }}" class="form-control" placeholder="Masalan: {{ $activeParent ? 'Telegram Reklama' : 'Reklama & Marketing' }}" required>
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    <input type="text" name="icon" id="category-icon-input" value="{{ old('icon', $model->icon ?: ($activeParent ? '✈️' : '📁')) }}" class="form-control text-center fs-4" placeholder="📁" maxlength="10">
                            ...
                            <div class="col-12">
                                ...
                                <textarea name="description" class="form-control" rows="3" placeholder="Ushbu xarajat kategoriyasiga nimalar kirishi haqida qisqacha izoh...">{{ old('description', $model->description) }}</textarea>
                            </div>

                            <!-- Tugmalar -->
                            <div class="col-12 pt-3 d-flex align-items-center justify-content-end gap-2 border-top">
                                <a href="{{ route('admin.expense-categories.index', ['parent_id' => $parentId ?? ($activeParent?->id)]) }}" class="btn btn-light border-0">Bekor qilish</a>
