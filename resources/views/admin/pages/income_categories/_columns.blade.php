                ...
                <tr>
                    ...
                    <td>
                        <div class="d-flex align-items-center gap-2">
                            <a href="{{ route('admin.income-categories.index', ['parent_id' => $data->id]) }}" class="text-dark fw-bold fs-14 text-decoration-none hover-primary">
                                {{ $data->name }}
                            </a>
                        </div>
                        @if ($data->description)
                    ...
                    <td>
                        <a href="{{ route('admin.income-categories.index', ['parent_id' => $data->id]) }}" class="badge {{ $childrenCount > 0 ? 'bg-soft-primary text-primary' : 'bg-light text-muted' }} px-3 py-2 text-decoration-none" title="Ichiga kirish">
                            <i class="feather-folder me-1"></i>
                            {{ $childrenCount > 0 ? $childrenCount . ' ta ichki bo\'lim' : 'Bo\'sh (0 ta)' }}
                        </a>
                    </td>
                    <td>
                        <form action="{{ route('admin.income-categories.toggle', $data) }}" method="POST" class="d-inline">
                            @csrf
                    ...
                    <td class="text-end">
                        <div class="d-flex align-items-center justify-content-end gap-1">
                            ...
                            <a href="{{ route('admin.income-categories.index', ['parent_id' => $data->id]) }}" 
                            ...
                            <a href="{{ route('admin.income-categories.edit', $data) }}" 
                            ...
                            <button type="button" 
                                    data-url="{{ route('admin.income-categories.destroy', $data) }}" 
                                    data-message="{{ $childrenCount > 0 ? 'DIQQAT: Ushbu kategoriyaning ichida ' . $childrenCount . ' ta ichki bo\'lim mavjud! O\'chirishni xohlaysizmi?' : 'Haqiqatan ham ushbu Tushum kategoriyasini o\'chirmoqchimisiz?' }}" 
                ...
                <tr>
                    <td colspan="7" class="text-center py-5">
                        <div class="text-muted">
                            ...
                            <h6>{{ isset($parentCategory) && $parentCategory ? '"' . $parentCategory->name . '" ichida hali ichki kategoriya yaratilmagan' : 'Hozircha tushum kategoriyasi qo\'shilmagan' }}</h6>
                            <p class="small mb-3">Yangi kategoriya qo'shish uchun quyidagi tugmani bosing</p>
                            @if(isset($parentCategory) && $parentCategory)
                                <a href="{{ route('admin.income-categories.create', ['parent_id' => $parentCategory->id]) }}" class="btn btn-sm btn-primary">
                                    <i class="feather-plus me-1"></i> Ushbu Guruhga Ichki Kategoriya Qo'shish
                                </a>
                            @else
                                <a href="{{ route('admin.income-categories.create') }}" class="btn btn-sm btn-primary">
