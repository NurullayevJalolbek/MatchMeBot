...
@section('title', 'Foydalanuvchilar Obunalari Tarixi')
...
@include('admin.pages.user_subscriptions._breadcrumb', [
            ...
            <div class="card stretch stretch-full">
                <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
                    <div class="d-flex align-items-center gap-2">
                        <h5 class="card-title mb-0">Obunalar Tarixi</h5>
                        <div class="btn-group btn-group-sm ms-3" role="group">
                            <a href="{{ route('admin.user-subscriptions.index') }}" class="btn {{ empty($currentStatus) ? 'btn-primary' : 'btn-light' }}">
                                Barchasi
                            </a>
                            <a href="{{ route('admin.user-subscriptions.index', ['status' => 'active']) }}" class="btn {{ $currentStatus === 'active' ? 'btn-success' : 'btn-light' }}">
                                <i class="feather-check-circle me-1"></i> Faol
                            </a>
                            <a href="{{ route('admin.user-subscriptions.index', ['status' => 'expired']) }}" class="btn {{ $currentStatus === 'expired' ? 'btn-secondary' : 'btn-light' }}">
                                <i class="feather-clock me-1"></i> Tugagan
                            </a>
                            <a href="{{ route('admin.user-subscriptions.index', ['status' => 'cancelled']) }}" class="btn {{ $currentStatus === 'cancelled' ? 'btn-danger' : 'btn-light' }}">
                ...
                <div class="card-body custom-card-action p-0">
                    @include('admin.pages.user_subscriptions._columns', ['datas' => $subscriptions ?? $datas])
                </div>
...
<script>
    (function ($) {
        ...
        // Obunani muddatidan oldin bekor qilish
        $(document).on('click', '.btn-cancel-sub', function (e) {
            e.preventDefault();
            const $btn = $(this);
            ...
            Swal.fire({
                ...
                text: user + " ning Premium maqomi darhol to'xtatiladi.",
                icon: "warning",
                    ...
                    axios.post(url)
                        .then(function (response) {
                            if (response.data && response.data.success) {
                                Toast.fire({
                                    icon: 'success',
                                    title: response.data.message || "Obuna bekor qilindi!"
                                });
