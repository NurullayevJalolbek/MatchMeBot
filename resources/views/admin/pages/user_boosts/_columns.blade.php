    ...
    <table class="table table-hover mb-0 align-middle">
        ...
        <tbody>
            @forelse ($datas as $index => $data)
                ...
                @endphp
                <tr id="boost-row-{{ $data->id }}">
                    <td>
                        <span class="text-muted fw-medium">{{ $datas->firstItem() + $index }}</span>
                    </td>
                    <td>
                        <div class="d-flex align-items-center gap-2.5">
                            <div class="avatar-text avatar-md bg-soft-warning text-warning rounded-circle fw-bold fs-14">
                            ...
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <div>
                                    ...
                                    <span class="badge bg-soft-warning text-warning px-1.5 py-0.5 fs-11">
                                        <i class="feather-zap me-1"></i> {{ $plan->hours }} soatlik
                                    </span>
                                </div>
                            </div>
                        @else
                    ...
                    <td class="text-end">
                        <div class="d-flex align-items-center justify-content-end gap-1">
                            @if ($isActive)
                                <button type="button" 
                                        data-url="{{ route('admin.user-boosts.cancel', $data) }}" 
                                        data-user="{{ $displayName }}"
                                        class="btn btn-sm btn-soft-warning border-0 btn-cancel-boost px-2.5 py-1.5" 
                                        title="Boostni muddatidan oldin bekor qilish">
                                    <i class="feather-slash me-1"></i> Bekor qilish
                                </button>
                            @endif

                            <button type="button" 
                                    data-url="{{ route('admin.user-boosts.destroy', $data) }}" 
                                    data-message="Haqiqatan ham ushbu boost tarixini o'chirmoqchimisiz?" 
                ...
                <tr>
                    <td colspan="8" class="text-center py-5">
                        <div class="text-muted">
                            ...
                            <h6>Boostlar tarixi mavjud emas</h6>
                            <p class="small mb-0">Hozircha hech qanday foydalanuvchi boost xizmati qayd etilmagan</p>
                        </div>
                    </td>
                </tr>
            @endforelse
